"""
Hand built technical indicators and feature engineering (the reference feature set
used by every capstone contestant; copied from the capstone so this package is self
contained).
...
def build_feature_panel(prices, market, tickers=None):
    ...
    tickers = tickers or sorted(
        t for t in prices["Ticker"].unique() if t not in BENCHMARKS
    )
    blocks = []
