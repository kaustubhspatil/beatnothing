...
class Membership:
    ...
    def __init__(self, path: Path | None = None):
        path = path or PIT_DIR / "sp500_membership_by_date.csv"
    ...
    def on(self, date) -> frozenset:
        ...
        if i < 0:
            raise ValueError(f"no membership data on or before {date}")
        return self._sets[i]
...
def coverage_report(membership: Membership, available_tickers, start, end) -> dict:
    ...
    """
    se = pd.read_csv(PIT_DIR / "sp500_ticker_start_end.csv", parse_dates=["start_date", "end_date"])
